...
def count_languages_and_save(file_pattern='Booking Stats.xlsx', output_excel='language_counts_result.xlsx', num_sheets=12):
    ...
    for file_path in file_paths:
        # Read the Excel file into a pandas DataFrame with all sheets
        xls = pd.ExcelFile(file_path)

        # Iterate through the first 'num_sheets' sheets in the Excel file
        for sheet_name in xls.sheet_names[:num_sheets]:
            # Read the sheet into a pandas DataFrame
            df = pd.read_excel(file_path, sheet_name)
            
            # Group by 'product_country' and 'language', then count the occurrences
            grouped_df = df.groupby(['product_country', 'language']).size().reset_index(name='count')
            
            # Iterate through each row and update the language_counts dictionary
            for index, row in grouped_df.iterrows():
                country = row['product_country']
                language = row['language']
                count = row['count']
                
                # Create a nested dictionary if the country is not in language_counts
                if country not in language_counts:
                    language_counts[country] = {}
                
                # Update the language count for the specific country
                language_counts[country][language] = language_counts[country].get(language, 0) + count

    # Convert the language_counts dictionary to a DataFrame
    result_df = pd.DataFrame([(country, language, count) for country, languages in language_counts.items() for language, count in languages.items()],
